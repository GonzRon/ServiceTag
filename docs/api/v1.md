...
**An edit replaces the whole record.** `PATCH /v1/assets/{id}`, `PATCH /v1/events/{id}`, and
`POST /v1/definitions` or `POST /v1/profiles` with an `id` are full replaces, not partial updates.
The body is the complete new state: an optional field left out is cleared, not kept, and a list
left out becomes empty. On an asset that includes `parentAssetId` — leave it out and a component
becomes a top-level asset. To change one field, read the record, change that field and send
everything back.

The MCP server in `tools/servicetag-mcp/` adds a convenience layer over this. Its `update_asset`
has partial-edit semantics: it reads the current asset, overlays the fields it was given, and
submits the complete replacement, so a field that is not mentioned keeps its value. Clearing a
field there is always explicit. That is the MCP tool's behaviour, not the API's.
