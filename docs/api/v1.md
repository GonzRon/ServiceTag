...
(`id`, `status`, `retiredOn`, `createdAt`, `templateKey`) are not in the command at all — retiring
and archiving stay on their own endpoints, and a PATCH never changes either. On an asset that
includes `parentAssetId` — leave it out and a component becomes a top-level asset.
