...
14. Beside it, Kotlin `assert` in connected tests (a no-op on ART; the pattern also catches an inline `runOnIdle { assert(` and leaves Compose's `.assert(` alone) — `grep -rnE '(^|[^.[:alnum:]_])assert\(' app/src/androidTest` → no output.
