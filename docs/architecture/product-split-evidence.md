...
## ServiceTag 1.1.0 — local automation API, workstation MCP server, conflict-safe backup merge (issue #46), 2026-09-21

**What shipped.** A loopback JSON API inside the app (`127.0.0.1:17337`, alive only while the Developer API screen is resumed, a fresh pairing code per visit as a bearer token); a workstation MCP server at `tools/servicetag-mcp/` (21 tools; unknown arguments refused centrally; edits overlay the current row; `retire_asset` monotonic); an additive backup import that plans before it writes and answers INSERT / IDENTICAL / CONFLICT / SKIPPED, never an update or a delete. `android.permission.INTERNET` added, with the owner's sentence on what it does and does not mean in the manifest, README and `docs/api/v1.md`. Classified MINOR under `docs/versioning.md`: 1.0.0 → 1.1.0, `versionCode` 11 → 12. Range `f588da3..b466e93`, 20 commits on `master`, each reviewed (six task reviews, scoped re-reviews per fix round, a whole-branch release review). The contract `docs/api/v1.md` had roughly 250 statements independently checked against the committed implementation, so it is a verified compatibility artifact rather than prose. Plan defects recorded, not papered over: the plan's own grep expected two occurrences of each import path in the contract (the correct text has three and four); the plan expected two `uses-permission` lines in the built artifact (there are three — the third is AndroidX's app-private receiver permission, already present in the 1.0.0 APK; the only new capability is `INTERNET`).

| proof | how | result |
|---|---|---|
| 1 unit gate from scratch | `--rerun-tasks` at `8eae323`, re-run at `b466e93` | 770 tests / 84 classes, 0 failures (core 33/394, app 42/316, nfc-core 6/50, nfc-android 3/10); Python 113 passed (21 tools) |
| 2 connected suite on `emulator-5554` | preserved pre-split set staged so the format-5 restore executed; at `8eae323`, carried to `b466e93` by tree identity of `app/src/main`, `core/src/main`, `libs` and the build files | 87 tests / 24 classes, 0 failures, 0 skipped (`DeveloperApiListenerTest` 3, `SettingsBackupEntryTest` 2, `PreservedSetRestoreTest` 1) |
| 3 structural facts | 41 anchored greps (one `ServerSocket(`, one quoted loopback literal, no outbound call site, no client socket, permissions NFC + INTERNET only, the merge sources free of delete/`updatedAt`/upsert, apply's guard order, five unique indices, the six ratified strings, format 5 / schema 5); ten untouched paths; submodule | all at their expected counts; the ten paths unchanged across the range; gitlink `7e0377a`, pin ok |
| 4 built artifact | `aapt2 dump badging` | `versionCode='12' versionName='1.1.0'` |
| 5 release signing dry run | `tools/release-dry-run.sh` with the published fingerprint | `RELEASE DRY RUN: PASS`, `fingerprint compare: matches` |
| 6 hygiene | the diff `f588da3..b466e93` and every commit message | no device id, e-mail, home path, UUID or real pairing code; single-line subjects; one author identity; one real fixture brand replaced by a fictional one before the push |
| 7 end-to-end on `emulator-5554` | clean install of the final debug build; Settings → Developer API by `uiautomator`; the code read from under its label; a connected MCP client through the server's own `adb forward` | 19/19: schemas strict on 21 tools; round trip (status, create → read back); a one-field edit, an all-`null` edit and a category change preserve every other field and the parent; `clear_fields` clears exactly one text or nullable field; unpaired, wrong-code and 422 refusals actionable; merge on the preserved set — plan writes nothing (61 insert / 8 skipped for attachments without a store), apply grows the store by exactly 61, re-plan all identical, a locally edited overlap is one CONFLICT and the apply then returns the report as data with zero writes |
| 8 contract, live | HTTP checks through the forward | 9/9: 401 empty (no token, wrong token), 404 envelope, 405, wrong verb on a sub-resource 404, 413 empty, a read row sent back verbatim as an edit → 400, the same row minus the five documented keys → 200, wrong content type on import → 415 |
| 9 fail-closed | three unknown-argument calls with a sensitive-looking value | refused; the value absent from every result and exception chain; the phone's own "Requests this session" counter 0 → 0; no forward created; leaving the screen closes the port and the tool says how to recover |
| 10 ordinary CI green | run 35670900969 on `b466e93` | success, jobs `build` + `mcp`, 0 annotations |

**Release.** `servicetag-v1.1.0` (annotated) → `b466e93`; run 35671147026 success after the owner's environment review; `ServiceTag-1.1.0.apk` 9,707,937 B, SHA-256 `c2a4460cdf9332f4f362f9e8fbf7729eb3a711d78e089a1af440b68a55984727`; download checksum == published checksum; `apksigner` verifies (v2), one signer; certificate == `RELEASE_CERT_SHA256` == the 1.0.0 signer; badging `1.1.0` / `12`; not debuggable.

**Development phone.** The published APK installed on the clean development phone (Android 17, NFC-capable, nothing of ours installed before); the APK pulled back from the phone has the release checksum. Proofs 7, 8 and 9 re-run there against the signed build through the real `adb forward`: **28/28**, the same results as on the emulator, including the merge sequence on the preserved set and the recovery message after leaving the screen. The app stays installed with its store cleared, so the Stage-A migration starts from empty. The production phone was not attached; its next install is this 1.1.0 (code 12 over its 10), which supersedes the pending 1.0.0 install.

**Issues.** #46 closed as completed; #44 updated (the plan-before-write core shipped; the Backup screen's merge mode, owner conflict resolution and duplicate review stay open); follow-ups #51 (Developer API screen: recents thumbnail, dead listener), #52 (readable 422 details), #53 (MCP polish), #54 (CI action pinning).

**ServiceTag 1.1.0 published: the automation bridge is in place; the data contract (format 5, schema 5) and the signing identity are unchanged.**

## ServiceTag 1.2.0 — operational maintenance: schedules, groups and rounds, local reminders, a Maintenance tab, scan-to-maintain, API/MCP (issues #4 #5 #11 #21 #24 #27 #28 #49 #50 #55), 2026-09-23

**What shipped.** Room schema 6 / backup format 6 / the merge over ten tables, with closures as immutable facts and derived state non-canonical (B01); the scheduling engine — fixed-calendar, completion-relative, meter-based and time-or-meter rules, `DUE_SOON`/`DUE`/`OVERDUE`, pause, archive and season handling, postpone kept apart from snooze, `ScheduleRecompute.rebuild` pure in (config, events, closures, membership, today, zone) (B02); maintenance groups with per-member truth, the open-instant membership basis and "Close this round" that never claims work was done (B03, B15); the provider-neutral `ReminderProvider` port and reconcile contract (B04); the platform plumbing — `POST_NOTIFICATIONS` requested on first schedule creation, no exact alarms, boot / time / time-zone receivers (B05); the local provider — inexact daily digest, WorkManager backstop, three-day overdue re-notify, persisted nonces, device-local snooze (B06); Done / Snooze / Open quick actions on notifications (B07); primary navigation Dashboard · Assets · Maintenance with due work, schedules, groups and health (B08); the scan sheet that lands on actionable maintenance for a bound tag (B09); reminder health with idempotent repair (B10); per-tag placement labels (B11); the API's maintenance surface (19 new routes, 40 documented) and the MCP server at 38 tools (B12); version 1.2.0 / `versionCode` 13 with the docs (B13); the schedule editor and the five operations (B14). Fifteen briefs, each implemented from its brief, reviewed on its real diff and merged fast-forward with CI green on the exact tip; the plan and briefs are at `docs/superpowers/plans/2026-09-22-servicetag-1.2/`, the spec at `docs/superpowers/specs/2026-09-22-servicetag-1.2-operational-maintenance.md` (rev 4.1).

| proof | how | result |
|---|---|---|
| 1 unit gate from scratch | `--rerun-tasks` at `12ef30f`; carried to `84a3244` by code identity (the two commits after it are `.md`-only; the two test classes that read the docs re-run there) | 1,190 tests / 133 classes, 0 failures (core 533/49, app 597/75, nfc-core 50/6, nfc-android 10/3); Python 197 (MCP, 38 tools) + 158 (bundle), all passing |
| 2 connected suite on `emulator-5554` | every `androidTest` package or class in its own invocation (22 invocations); the owner's preserved format-5 set staged so the restore proof executed; at `12ef30f`, carried by code identity | 131 tests / 35 classes — every `@Test` in the tree — 0 failures, 0 skipped; run inside the after-20:00-local window that used to break two of them |
| 3 structural facts (§16.4) | anchored greps at the tip | no `SCHEDULE_EXACT_ALARM` / `USE_EXACT_ALARM`; `POST_NOTIFICATIONS` 1; `RECEIVE_BOOT_COMPLETED` 1; 6 receivers, none exported; one channel-creation site; no `PendingIntent` without `FLAG_IMMUTABLE`; the forward-only migration row; 1.2.0 / 13 / schema 6 / format 6; the 2.6 tombstones present; `androidx.work` absent from `:core`; gitlink `7e0377a` |
| 4 format-5 restore (§16.5) | `PreservedSetRestoreTest` on the owner's real 2026-09-18 export, on the emulator only | restores through `importBackupReplace` onto schema 6; link counts equal; the re-exported `externalLinks` byte-identical across the 5 → 6 round trip |
| 5 end-to-end on `emulator-5554` (§16.6) | clean install of the tip's debug build; Settings → Developer API by `uiautomator`; a connected MCP client through the server's own `adb forward`; then the Backup screen's export into a SAF folder and that export planned back | 26/26: pair; status 1.2.0; two assets → a group of two → a group-targeted QUICK schedule due today, one `list_due` item, 2 required / 0 complete; one member completes (1 of 2, still due); the same member again 409; `closedOn` tomorrow and before the open date 422 and nothing written; close → 201 and one closure; a repeated close closes the newly opened next round (two closures, distinct occurrences — spec §2.4); a completion after the closes lands on the round now current; `clear_fields`; the argument guard; archive → gone from due work, closures kept. Export format 6; `import_merge(plan_only)`: applicable, 0 conflicts, every row IDENTICAL (assets 2, events 2, groups 1, schedules 1, closures 2); status unchanged; `pm clear` |
| 6 hygiene (§16.7) | the diff `1f963f8..84a3244` and its 89 commit messages | one author identity; single-line subjects; no attribution; no e-mail, home path, device id, pairing code or private noun |
| 7 badging (§16.8) | `aapt2 dump badging` on the debug build, repeated on the artifact below | `versionCode='13' versionName='1.2.0'`, `targetSdk` 36; exactly the eight permissions of the `RECEIVE_BOOT_COMPLETED` ruling (NFC, INTERNET, POST_NOTIFICATIONS, RECEIVE_BOOT_COMPLETED, ACCESS_NETWORK_STATE, FOREGROUND_SERVICE, WAKE_LOCK, the AndroidX app-private receiver permission), no exact-alarm permission |
| 8 reviews | whole-branch release review at `7813614` and a scoped re-review of the fix range `7813614..12ef30f`, both on the most capable model | RELEASE / RELEASE, 0 blocking; every brief reviewed on its real diff, fix rounds scoped and re-reviewed |
| 9 ordinary CI green | run 35819156892 on `84a3244` (and 35817352402 on `4b9e01e`; every merged tip before it green in turn) | success, jobs `build` + `mcp` + `bundle`, 0 annotations |

**Found at the gate.** The connected sweep, run after 20:00 local, exposed a latent defect rather than a merge fault: D-27's pin floor read `updated_at` at UTC while today and the anchor are the owner's dates, so an evening-created schedule floored on tomorrow and its first occurrence moved a whole interval out. Ruled a product defect and fixed on lane `1.2-fix-shell` (the zone is a required pure input of `rebuild`; `RecomputeSchedules` supplies the device zone), reviewed twice, spec §2.1/§6 and the plan amended. The plan's §16.6 text then over-specified two refusals: a repeated `close_round` closes the *next* round because spec §2.4 opens it the moment the previous one is closed, so the proof asserts that and the API doc says it; a first `servicetag-v1.2.0` tag on `4b9e01e` was retired unpublished while that was settled and re-cut on `84a3244` with no code change.

**Release.** `servicetag-v1.2.0` (annotated) → `84a3244`; run 35819328082 success after the owner's environment review (a first tag on `4b9e01e` was retired unpublished, see above); `ServiceTag-1.2.0.apk` 10,615,429 B, SHA-256 `027c28cca454b89af31fc592f50e027864012de94781510391257f41eb486081`; download checksum == published checksum; `apksigner` verifies (v2), one signer; certificate == `RELEASE_CERT_SHA256` == the 1.1.0 signer; badging `1.2.0` / `13`, `targetSdk` 36; exactly the eight permissions; not debuggable.

**Development phone.** The verified APK installed over the development phone's signed 1.1.0 **in place** (`adb install -r`; `versionCode` 12 → 13 with the same `firstInstallTime` and UID); the APK pulled back from the phone has the release checksum. The Stage-A data survived the schema 5 → 6 migration: every 1.1.0 table keeps its row count (assets 41, definitions 23, events 26, profiles 67, tags 3, links 1), `list_assets` and `list_tag_bindings` are identical before and after, and the 26 events are identical in every shared field — the only difference is the three new event fields (`scheduleId`, `occurrenceOn`, `detailsPending`) at their defaults on every row; the new tables start empty. The group flow re-run there through the real `adb forward` against the signed build: **27/27**, the same results as on the emulator, the two proof assets archived afterwards. Hazard 9 on a real Stage-A asset bound to two tags: one schedule, listed exactly once. The groups UI, driven by `uiautomator` on a seeded group: the Maintenance tab shows the due schedule with "1 of 2 complete" and the group under "Maintenance groups" (archived groups listed with an ARCHIVED chip); the group screen shows both members, "1 of 2 complete", Complete selected / Complete all and Close this round; the schedule screen adds Postpone and Snooze — all ratified strings; the seed archived afterwards. The S4 probe's log was pulled (next-day re-arm and the first backstop run recorded) and the probe uninstalled; the phone stays on 1.2.0 with its data. The production phone was not touched; its install is the owner's separate decision.

**Issues.** #4, #5, #11, #21, #24, #27, #28, #49, #50 and #55 closed as completed, each with a closing comment naming what shipped, the briefs, the release facts and the proofs; #24 carries the `RECEIVE_BOOT_COMPLETED` reading of its AC 3 and the S4 follow-up; #44 updated (the merge now spans format 6's ten tables; the Backup screen's merge mode, conflict resolution and duplicate review stay open).

**Known limits and owner flags.** The D-27 floor now uses the device zone (reversible by ruling). `close_round` is not idempotent by spec — a retried close skips the newly opened round, immutably; tightening options are recorded for 1.2.1. `ZoneId.of("America/New_York")` names the negative-offset zone in four JVM test files. `lifecycleChangedAt` is deferred to 1.3. Post-1.2 tidies: `HealthScreenTest` inherits its neighbour's store; a mis-cited invariant in `DashboardAttentionTest`; the device zone declared explicitly at the `RecomputeSchedules`/`AppGraph` seam. *Superseded in part by 1.2.1 (2026-09-23): the D-27 device-zone floor was ratified and its zone made an explicit dependency; `close_round` now refuses a round before its due-minus-lead window, which closes the same-day retry hole except when the lead is at least the interval (a documented known limit); `America/New_York` is gone from the tests (a neutral zone, `America/Toronto` for the one transition case, ratified); the three post-1.2 tidies shipped.*
...
## ServiceTag 1.2.1 — hardening patch: the close-round window guard and the explicit device zone, 2026-09-23

**What shipped.** `close_round` refuses to close a maintenance round before the round's due-minus-lead window (409 `OCCURRENCE_NOT_YET_OPEN`, nothing written; the boundary day allowed; the UI's "Close this round" offered on the same arithmetic), which closes the immediate-retry hole a first close opened — with one documented known limit (a lead at least as long as the interval leaves the same-day retry unguarded; the occurrence key on the call is the follow-up) and one consequence ratified by the owner (a postponed round waits for its postponed window; clearing the postponement is the ungated way out). The device zone became a required, explicit dependency at the `RecomputeSchedules` seam with D-27's device-local floor and every scheduling result unchanged. Test tidies: `HealthScreenTest` starts from a fresh install, a mis-cited invariant corrected, `America/New_York` replaced by neutral zones. Version 1.2.1 / `versionCode` 14; schema 6 / format 6 unchanged; no new user-visible string. Two lanes (`docs/superpowers/plans/2026-09-23-servicetag-1.2.1-hardening/`), each reviewed on its real diff with scoped re-reviews; whole-range release review RELEASE, 0 blocking. The tag also carries `tools/servicetag-schedules` (Stage B, above).

| proof | how | result |
|---|---|---|
| 1 unit gate from scratch | `--rerun-tasks` at the tip | 1,200 tests / 133 classes, 0 failures (core 540/49, app 600/75, nfc-core 50/6, nfc-android 10/3); python 197 + 158 + 92 |
| 2 connected suite on `emulator-5554` | every package or class in its own invocation (22) | 131 / 35, 0 failures, 0 skipped |
| 3 end-to-end on `emulator-5554` | clean install; MCP client through the server's own `adb forward`; the Backup screen's export planned back | 27/27 including the new refusal on the retried close (one closure stays); export format 6; plan-only merge all IDENTICAL |
| 4 structural facts, hygiene, CI | the 1.2 greps; the range `84a3244..c3e691c` (41 commits, one author, no bodies, no private data); CI run 35882994081 | all hold; exact-alarm 0, receivers 6/0 exported, 1.2.1 / 14, `America/New_York` absent from Kotlin |

**Release.** `servicetag-v1.2.1` (annotated) → `c3e691c`; run 35883189356 success after the owner's environment review; `ServiceTag-1.2.1.apk` 10,615,433 B, SHA-256 `3aff7fbc5910f99ed8268576b1f3fd9fc0809284455ac31c5547af596c070afb`; download checksum == published; `apksigner` verifies (v2), one signer; certificate == `RELEASE_CERT_SHA256` == the 1.2.0 signer; badging `1.2.1` / `14`; the eight permissions; not debuggable.

**Development phone.** The verified APK installed over the signed 1.2.0 **in place** (code 13 → 14, same `firstInstallTime` and UID; the APK pulled back has the release checksum). The Stage-B estate survived unchanged: every table's row count identical before and after (46 schedules, 6 groups, 45 assets, 29 events, 67 profiles, 23 definitions, 3 tags, 2 closures, 1 link), and the schedule, group, asset, tag-binding and event lists identical field by field. The group flow re-run there through the real `adb forward`: every product check passed, including the retried close refused with `OCCURRENCE_NOT_YET_OPEN` and one closure kept; hazard 9 on the real two-tag asset: its schedules listed once each. Two harness lines reported FAIL for harness reasons only — an expected-version literal still set to 1.2.0, and a count that included an archived proof schedule from the 1.2.0 day — both corrected in the harness afterwards. The production phone was not touched.

**ServiceTag 1.2.1 published: the same maintenance subsystem, one guard tighter, one dependency explicit, no data or contract change.**

## ServiceTag 1.3.0 — share intake: a URL, a document, an image or a note link shared from any app lands on an asset (issue #43), 2026-09-23

**What shipped.** ServiceTag is an Android share target (`ACTION_SEND`, ten MIME types, one exported activity `share.ShareIntakeActivity` — the exported set is now exactly three activities by name, amending #35 AC 3 per ruling D-14). A shared URL or note link becomes an **asset reference** (new Room table `asset_reference`, schema 7; backup format 7, forward-only; the eleventh merge table, a diverged same-URL pair is SKIPPED per D-18 C); a shared document or image is copied into the attachment folder through the shipped `AddAttachment` (declared size checked before the copy, an undeclared size probed for one byte, nothing staged on cancel or refusal). The asset screen gains a **References** section with Open, Edit, Remove and **Add link**, and the Documents row shows a shared document's description (D-19). The loopback API gains `/v1/references` (list, POST, PATCH; five codes; no DELETE) and the MCP server three tools (41; package 1.2.0). The Dashboard's search box moved to the Assets screen with two ratified hints (B07). Fifty-two ratified strings; the `LinkLauncher` refusal is the ratified "No app can open this link" with no URI, shown once. Versions: `versionName 1.3.0`, `versionCode 15`; the four number pins and the release documents are asserted by `VersionAgreementTest`.

**Process.** Seven briefs (B01 schema/format/merge, B02 core references domain, B07 search move, B05 API/MCP, B03 share target and intake, B04 References section, B06 version/docs) in five waves with two lanes; one task review per brief and one scoped re-review per fix round until the owner's review-budget ruling (2026-09-23, `docs/superpowers/planning-policy.md`), after which B06 and the release fix round closed by inspection; one whole-branch release review (READY TO TAG, 0 blocking, 2 should-fix fixed in one round: the undeclared-size empty stream, the double missing-handler notice). Range `6b45021..d1d0460` (release tip; the code tip is 32488ca, docs after it); every commit by one author, no bodies, no attribution, no private data.

| proof | how | result |
|---|---|---|
| R1 unit gate from scratch | `--rerun-tasks` at the tip, four modules | **1,379 tests / 150 suites, 0 failures, 0 skips** (core 613/58, app 706/83, nfc-core 50/6, nfc-android 10/3) — equals the versioning row |
| R2 connected suite on `emulator-5554` | whole suite, `ANDROID_SERIAL` pinned | **167 / 0 failures / 0 skips at the last wave's tip (064d04a)**; the release fix round touched one connected class, `ReferencesSectionTest`, re-run 12/0 at its tip; no connected test changed after that. Not re-run at the release tip: the owner ordered no further emulator runs (2026-09-23) |
| R3 the three Python suites | `uv run --frozen`, plus a cold-cache run for the schedules lock | mcp 211, bundle 158, schedules 92; cold cache 92 |
| R4 share flows by intent on `emulator-5554` | `am start -a SEND` with `--es`/`--eu`; MediaStore URIs carry the shell's grant; one real Files-app share (R4.F) proves a genuine SAF grant; names typed after disabling Gboard stylus handwriting (§18.26) | filter (pdf/plain/jpeg answered, SEND_MULTIPLE not); R4.F; 10a/10b; 1 (bare URL, typed name → WEB_URL); 1c cancelled; 4 note-link (name prefilled, NOTE_LINK); 5 blocked scheme; 7 not-a-link → note event; 8 refused `file://` stream; process death → nothing written; 9 no assets; 6 unknown scheme → "Save this link?" → OTHER; **2 document and 3 image retired**: their OS boundary (an external stream with a genuine grant reaching the byte-share form) is proven by R4.F and 10a, and the save-to-attachment step is in-process coverage (`ShareIntakeScreenTest`, `SafTreeAttachmentStoreContractTest`, `AttachmentsDeviceProofTest`); the black-box save journey is retired under the testing-hierarchy ruling (#62) |
| R5 archives | — | not run as emulator journeys (owner order); the contracts are JVM-covered (format 7 codec and merge tests, the `BackupNewerFormat` guard) and `PreservedSetRestoreTest` (format 5 → 7 on the emulator) ran green inside R2 |
| R6 two-generation D-18 | — | not run as an emulator journey (owner order); D-18 C (a diverged shared-URL pair SKIPPED, same-id/different-content a blocking CONFLICT) is covered by B01's `MergePlanner` reference tests |
| R7 API/MCP e2e | the sequence through the MCP client on `emulator-5554` | create 201 (id, kind `WEB_URL`) and list; duplicate 409 `REFERENCE_URI_TAKEN`; blocked and unknown scheme 422 `REFERENCE_SCHEME_BLOCKED` (never a confirmation); 2,049-char URI 422 `REFERENCE_URI_INVALID`; blank name 422 `REFERENCE_NAME_REQUIRED`; `uri=` on update rejected; `DELETE /v1/references/{id}` 405. The update-leaves-URI step was not exercised end to end (covered by `ReferenceRoutesTest` and the MCP suite) |
| R8 structural checks | 28 lines against base `6b45021` at the tip | **all PASS** (exported set parsed = three activities, no other exported component; BROWSABLE file-wide 1 / added 0; mimeType 11; no provenance; tombstones untouched and unread; 41 tools; gitlink `7e0377a`) |
| R9 hygiene | added lines over the range | no e-mail (the only hits are the fictional fixture and the `@mcp.tool` decorator), no home path, no serial but `emulator-5554`, no attribution, 0 bodies |
| CI | every pushed tip | green: 35911846874, 35921713415, 35928945419, 35930494806, 35932876850, 35933655864 |

**Known limits, recorded.** A chosen asset deleted while the intake is open shows the no-assets sentence; a `text/uri-list` with no usable line reads as "Could not read what was shared"; a text share whose only URI token is structurally invalid leaves the form with "That is not a link." and Cancel; `OwnerMissing`/`NoSuchReference`/a refused write are silent on the References section (no ratified sentence). `tools/servicetag-schedules/uv.lock` still names servicetag-mcp 1.1.1 as a directory dependency (harmless under `--frozen` and on a cold cache). A tracked `app/release/app-release.apk` predates the split (owner's).

**Release.** `servicetag-v1.3.0` (annotated) → `d1d0460`; run 35935412320 success after the owner's environment approval; `ServiceTag-1.3.0.apk` 10,714,329 B, SHA-256 `a4ca5ec8460b9d1821b6dbfbf2b665d61929ea6a80d0eeb1a59a856be07f3442` (download == published), apksigner v2, exactly one signer == `RELEASE_CERT_SHA256`, badging 1.3.0 / 15, the eight permissions and no exact-alarm, not debuggable. CI green on the release tip (35935374944).

**Development phone.** The verified APK installed over the signed 1.2.1 **in place** (code 14 → 15, same `firstInstallTime` and UID); the app launches; every table count identical before and after (assets 47, tags 3, links 1, definitions 23, profiles 67, events 32, attachments 0, groups 7, schedules 48, closures 3) with `assetReferences` 0 before anything is added; schema 7 / format 7 reported by the API. The URL share, document share and `joplin://` round trip on the phone were not run: the owner closed the release after the upgrade smoke (testing-hierarchy ruling, #62). The production phone is untouched.

**ServiceTag 1.3.0 published: share from any app into an asset, references on the asset screen, API and MCP reference tools, schema 7 / format 7.**

## ServiceTag 1.4.0 — operating seasons, maintenance service policy and a maintenance break, operational condition and derived health (issues #14 #60 #61 #64)

*Stub, written by B15 before the release proofs ran. Every "Result" below, the date in this heading, and the Reviews, Release, Development phone and Issues paragraphs are the controller's to fill at the exact release tip. The proof list is the runnable form of master plan §17 (`docs/superpowers/plans/2026-09-24-servicetag-1.4/master-plan.md`) and of `docs/release-proofs.md`; every command is one physical line, run from the repository root, and `<base>` is the 1.4 implementation base (`537c44e`, the plan tip B01 was cut from).*

**What shipped.** Room schema 8 / backup format 8: five `asset` columns, `maintenance_schedule` recreated with `service_policy`, `policy_offset_days` and `rule_changed_at` in place of the three season columns, `schedule_state` recreated as derived, three new tables (`asset_season_activation`, `asset_condition`, `health_subject`), one legacy season mapping shared by the migration, the format ≤7 decoder, the API and the MCP, and #64's pin floor on `rule_changed_at` (B01); the season context and the service-policy engine — the actionable date, `DEFERRED`, dormancy and `quiet`, with reads that never write (B02); format 8, the old-archive upgrade and merge tables 12–14 (B03); the season-mode, maintenance-break, activation and policy commands (B04); the health engine, computed at read time and never stored (B05); condition facts and the health configuration (B06); the shared read models (B07); the schedule editor's policy question (B08); `/v1`'s new routes, the two command forms and the deprecated inputs (B09); the asset and health-subject editors (B10); the MCP server at 55 tools and the schedules loader (B11); condition and health on screen, the scan sheet and the offers (B12); the dashboard and maintenance rows with their why-lines (B13); asset detail's Condition, Health and Season sections (B14); version 1.4.0 / `versionCode` 16 and the documents (B15); and, from B15's lint gate, a fix for a crash on API 26–33 when opening schedule detail (shipped since 1.2.0) or the schedule editor — both defaulted a date to `LocalDate.EPOCH`, a field that exists only from API 34. Fifteen briefs in nine waves, at most two lanes; plan and briefs at `docs/superpowers/plans/2026-09-24-servicetag-1.4/`, the spec at `docs/superpowers/specs/2026-09-24-servicetag-1.4-seasons-policy-condition-health.md` (rev 2.2 with O-9; strings S1–S143).

**The proof list.** Run by the controller on the tagged commit 750efc8 (2026-09-25): items 1–21 all PASS — R1 from scratch `:core` 817 / `:app` 968 / nfc-core 50 / nfc-android 10, 0 failures, 0 skips, `ReleaseProofPolicyTest` 2/0; lint 0 errors (56 warnings, 1 hint, nothing suppressed); R2 the whole connected suite on emulator-5554 with the preserved set staged: 215 tests, 47 classes, 0 failures, 0 skips, `ShareBoundaryTest` 3, `Format7RestoreContractTest` 2, `PreservedSetRestoreTest` 1 (ran); R3 mcp 315 / bundle 158 (no diff) / schedules 106; R5 three activities and `other 0 []`; the numbers 1 each; schemas 1–7 untouched and 8.json present; `MergeTable` 1; `:core` Android-free; the `assert` grep, invariant 129, 104, 111, the facts-immutable grep, the untouched list, the tombstones and the stale-prose greps all clean; R6 hygiene: no e-mail, no home path, no serial but the emulator's, no attribution, one author, empty bodies, `submodule pin ok`. Items 22–25 are under Release and Development phone below.

1. R1, unit gate from scratch — `./gradlew --rerun-tasks :nfc-core:test :nfc-android:testDebugUnitTest :core:test :app:testDebugUnitTest` → 0 failures, 0 skips; the counts equal the baseline plus each brief's recorded delta (B15's own run: 1,843 tests / 218 suites — `:core` 817, `:app` 966, nfc-core 50, nfc-android 10). `VersionAgreementTest` (the numbers agree), `ReleaseProofPolicyTest` and the R5 manifest classes are inside it.
2. Lint — `./gradlew :app:lintDebug` → no error; no plurals suppression (the day forms are four plain strings read by id under English rules, B12 ruling), so any error is a real finding.
3. R2, the connected suite, after staging the preserved set — `adb -s emulator-5554 push <the format-5 preserved set> /data/local/tmp/servicetag-proof-data.zip`, then `ANDROID_SERIAL=emulator-5554 ./gradlew :app:connectedDebugAndroidTest` → 0 failures, 0 skips; the new Compose classes present.
4. The format-7 import proof (master §17.8) — `Format7RestoreContractTest` inside item 3's run, over B01's committed golden format-7 archive → equal counts, CALENDAR iff both `MM-DD` were set, no break, the §4.1 mapping on every schedule, `ruleChangedAt = updatedAt`, empty new lists; exported again it is format 8 and plans IDENTICAL.
5. The format-5 restore — `PreservedSetRestoreTest` inside item 3's run → the preserved set restores onto schema 8.
6. R3, the Python suites — `root=$(git rev-parse --show-toplevel) && (cd "$root/tools/servicetag-mcp" && uv run --frozen pytest) && (cd "$root/tools/servicetag-bundle" && uv run --frozen pytest) && (cd "$root/tools/servicetag-schedules" && uv run --frozen pytest)` → all pass (B15's run: mcp 315, bundle 158, schedules 106); `grep -c '^@mcp\.tool' tools/servicetag-mcp/src/servicetag_mcp/server.py` → 55; the bundle byte-identical (`git diff --stat <base>..HEAD -- tools/servicetag-bundle/` → empty).
7. R4 — R2's `ShareBoundaryTest`, unchanged and alone in the R4 row → 3 cases, each under 20 s.
8. R5 — the runbook's parser line (`docs/release-proofs.md`, "R5's parser line") → exit 0, the three activities and `other 0 []`; 1.4 adds no permission and no exported component.
9. The numbers — `grep -cE '^[[:space:]]*version = 8,$' app/src/main/kotlin/com/loosecannon/servicetag/data/room/AppDatabase.kt`, `grep -cE '^[[:space:]]*const val SCHEMA_VERSION = 8$' app/src/main/kotlin/com/loosecannon/servicetag/di/AppGraph.kt`, `grep -cE '^[[:space:]]*const val FORMAT_VERSION = 8$' core/src/main/kotlin/com/loosecannon/servicetag/core/backup/BackupCodec.kt`, `grep -cE '^[[:space:]]*versionCode = 16$' app/build.gradle.kts`, `grep -cE '^[[:space:]]*versionName = "1\.4\.0"$' app/build.gradle.kts` → 1 each; `grep -cE '^\| 1\.4\.0 \| 16 \|' docs/versioning.md` → 1.
...
11. The merge — `grep -c '^enum class MergeTable' core/src/main/kotlin/com/loosecannon/servicetag/core/merge/MergePlan.kt` → 1 (the review reads the fourteen members, `REFERENCES` then the three new); `grep -cE '^[[:space:]]*UPDATE[,[:space:]]*$' core/src/main/kotlin/com/loosecannon/servicetag/core/merge/MergePlan.kt` → 0.
12. `:core` Android-free — `grep -rlE '^import (android|androidx)\.' core/src/main` → no output.
13. The retired-token tripwire — `ReleaseProofPolicyTest` inside item 1 (no file under `tools/`, `app/src/androidTest/`, `share-test-sender/` or `.github/` names a screen-driving tool; R4 is `ShareBoundaryTest` alone) → green; the retired season fields have no engine or reminder reader: `grep -rnE '\bseason(Behavior|Reentry|ReentryOffsetDays)\b' core/src/main/kotlin/com/loosecannon/servicetag/core/schedule core/src/main/kotlin/com/loosecannon/servicetag/core/reminders core/src/main/kotlin/com/loosecannon/servicetag/core/health` → no output; the three retired strings: `grep -rnF "Pause with the asset's season" app/src/main`, `grep -rnF 'Remind me year round' app/src/main`, `grep -rnF 'Off means this asset is only in use' app/src/main` → no output each.
14. Beside it, Kotlin `assert` in connected tests — the standing R6 line in `docs/release-proofs.md` (a no-op on ART; the pattern also catches an inline `runOnIdle { assert(` and leaves Compose's `.assert(` alone) — `grep -rnE '(^|[^.[:alnum:]_])assert\(' app/src/androidTest` → no output.
15. Invariant 129 — `git diff <base>..HEAD -- app/src/main core/src/main | grep -niE '^\+.*\b(assembly|assemblies|installed_?component|stock)\b'` → no output.
16. No reminder code evaluates a season (inv. 104) — `grep -rnE '(Season\.inSeason|SeasonContext|ServicePolicyEngine|blackout(Start|End)Mmdd|season(Start|End)Mmdd)' core/src/main/kotlin/com/loosecannon/servicetag/core/reminders app/src/main/kotlin/com/loosecannon/servicetag/reminders` → no output.
17. Facts immutable — `grep -rnE '(UPDATE|DELETE)[[:space:]]+(FROM[[:space:]]+)?.?(asset_condition|asset_season_activation)\b' app/src/main core/src/main` → no output.
18. No health value persisted (inv. 111) — `grep -rniE '(health_score|health_band|health_state|healthScore|healthBand)' app/src/main/kotlin/com/loosecannon/servicetag/data core/src/main/kotlin/com/loosecannon/servicetag/core/backup core/src/main/kotlin/com/loosecannon/servicetag/core/merge` → no output.
19. The untouched list — `git diff --stat <base>..HEAD -- libs/ tools/servicetag-bundle/ share-test-sender/ app/src/main/AndroidManifest.xml core/src/main/kotlin/com/loosecannon/servicetag/core/nfc core/src/main/kotlin/com/loosecannon/servicetag/core/journal app/src/main/kotlin/com/loosecannon/servicetag/share` → empty; the tombstones: `git diff --stat <base>..HEAD -- core/src/main/kotlin/com/loosecannon/servicetag/core/model/ExternalLink.kt app/src/main/kotlin/com/loosecannon/servicetag/data/room/entities/ExternalLinkEntity.kt app/src/main/kotlin/com/loosecannon/servicetag/data/room/dao/ExternalLinkDao.kt` → empty.
20. The stale prose gone — `grep -ci 'forty-one' tools/servicetag-mcp/README.md` → 0; `grep -cE '(format \*\*1–7\*\*|\*\*format 1–7\*\*)' docs/api/v1.md` → 0; `grep -ci 'the eleven tables' docs/api/v1.md` → 0.
21. R6, hygiene — the runbook's R6 greps over `<base>..HEAD` → each prints nothing, or the count stated; `bash tools/check-submodule-pin.sh` → `submodule pin ok`.
22. R7, the one signed-APK upgrade smoke on the development phone (`docs/release-proofs.md`, "R7 in full", including its schema-and-format line) — `adb install -r <verified apk>` over the installed 1.3.0 → code 15 → 16, the same UID and `firstInstallTime`; **(i)** `/v1/status` counts and a per-table hash of every list route before and after, each object hashed over the 1.3 key set, a schedule's pre-install triple passed through `toLegacy(toPolicy(…))` and every other key verbatim → identical, the three new count keys 0, `schemaVersion` and `backupFormatVersion` 8, and spec §12.3's per-row checks (CALENDAR iff both `MM-DD`, no break, the §4.1 mapping, empty new lists) read off the same responses; **(ii)** is item 23; **(iii)** is item 5. No export and no owner action.
23. The Stage-B re-plan — `servicetag-schedules plan <the owner's manifest> --code <code>` against the upgraded development phone → exit 0, every entry IDENTICAL.
24. MCP on the phone — `status` → 8 / 8; one read of each new read tool answers; no write beyond what the owner authorises.
25. CI and the release — the jobs `build`, `mcp`, `bundle`, `schedules` green on the exact commit with 0 annotations; annotated tag `servicetag-v1.4.0`; after the owner's environment approval: `sha256sum -c`, one signer equal to `RELEASE_CERT_SHA256`, badging `1.4.0` / `16`, not debuggable, the permission set equal to 1.3.0's.

**Reviews.** *(controller: the fifteen task reviews and their scoped re-reviews, and the one whole-branch review at the final tip.)*

**Known limits, recorded.** A same-day season START or END recorded on this device can sort before a merged activation row of the same date whose `createdAt` is later because the other device's clock runs ahead: the call succeeds, the phase does not change, and "already started" or "already ended" admits the same action again — rare, reachable only once development-to-production merges begin (B04 review, M7; spec §3.3's ordering). The legacy form's PRE_SERVICE refusal on a schedule `PATCH` reads the stored row outside `SaveSchedule`'s transaction, a theoretical race that closing would take a `:core` change for, not made in 1.4 (B09 review, M5). *(controller: any further limit from the whole-branch review.)* Asset detail draws no line for "in the break now" (no ratified sentence exists; the schedules' why-lines carry the quiet state). Invariant 119 (nothing hides behind an aggregate on the dashboard) applies to the unfiltered dashboard: with a status-only filter a DOWN asset's own row may be hidden while its schedule row's health line still shows.

**Release.** CI run 36134774892 on 750efc8: `build`, `mcp`, `bundle`, `schedules` green, 0 annotations. The release dry run built and signed `app-release.apk`, version 1.4.0, checksum written. Annotated tag `servicetag-v1.4.0` → 750efc8; release run 36135503275 completed after the owner's environment approval. Asset `ServiceTag-1.4.0.apk`, sha256 `2b916ab2…20f338c9` equal to the published `.sha256`; the signer is the same certificate as the installed 1.3.0 build (fingerprint `894afa76…`, the ServiceTag release key since 2.5); `versionCode` 16 / `versionName` 1.4.0; the eight `uses-permission` lines identical to 1.3.0.

**Development phone.** `adb install -r` of the verified APK over 1.3.0 (code 15): 1.4.0 / 16 in place, `firstInstallTime` and the app UID unchanged. R7 (i): the API snapshot before and after (routes and the script recorded beside the results: assets, tag bindings, groups, schedules on their identity and rule keys with the season triple normalised, events and references per asset) — every table identical on the 1.3 key set; the asset response gained exactly `seasonMode`, `blackoutStartMmdd`, `blackoutEndMmdd`, `healthAggregation`, `healthPrimarySubjectId`; every 1.3 count identical (assets 47, tags 3, links 1, definitions 23, profiles 67, events 32, attachments 8, groups 7, schedules 48, closures 3); the new counts `seasonActivations`, `assetConditions`, `healthSubjects` are 0; `/v1/status` reports app 1.4.0, schema 8, format 8. R7 (ii): the Stage-B manifest re-planned through the loader: `CREATE=0 IDENTICAL=47 CONFLICT=0 ERROR=0`. R7 (iii): the preserved 2026-09-18 set restored onto schema 8 in R2. Item 24: the MCP's `status` answers 8 / 8 and `get_season`, `list_conditions`, `get_health`, `list_health_subjects`, `list_attention` each answered; no write was made. The production phone was not touched.

**Issues.** *(controller: #14, #60, #61 and #64, each closed with what shipped, the briefs and the proofs.)*

## ServiceTag 1.4.1 — reminder delivery repaired, the editor keeps a one-tap schedule's quick action (issues #80 #81; versionCode 17; 2026-09-25)

**What shipped.** A PATCH on 1.4.0. **#80:** a schedule created through the automation API with `providers` omitted stored reminders on and no provider row, so nothing was ever delivered (`BuildReminderSubjects` keeps only rows with an enabled provider); the Stage-B loader and the MCP `create_schedule` both omitted the key, which is how both phones held 43 such rows. A `/v1` create that omits `providers` now gets one `LOCAL` row with `enabled = remindersEnabled` (an explicit `[]` still stores none; PATCH unchanged, a `null` still refused); the MCP and the loader send the row explicitly; and one idempotent use case, `RepairScheduleProviders`, repairs the shipped rows — Reminder Health's "Fix reminder delivery" in the app, `POST /v1/repairs/schedule-providers/{plan,apply}` on the API, `repair_schedule_providers(plan_only=True)` on the MCP (56 tools) — adding `LOCAL enabled` to ACTIVE, reminders-on, providerless schedules and moving nothing but `providers` and `updatedAt`; never `RepairAction.Automatic`; a non-empty set with nothing enabled is `SCHEDULE_PROVIDER_DISABLED`, surfaced and never swept. **#81:** the editor kept the profile only in FORM mode since 1.2; the profile now survives load, edit and save in either mode, the picker is drawn under One tap as `Quick action`, and a `None` row clears it. Room schema 8 / backup format 8 unchanged. Plan, briefs and the owner's rulings: `docs/superpowers/plans/2026-09-25-servicetag-1.4.1/` (`c9164cc`); four briefs in three waves, at most two lanes, one task review each; tip `80a5c9e` = tag `servicetag-v1.4.1`. Also fixed before wave 1: the README rewrite `966aa91` had broken three `VersionAgreementTest` cases while CI stayed green because the README was not a declared test input — tests re-anchored, README / `docs/versioning.md` / `docs/design` / `docs/api` declared inputs (`66f9297`).

**The proof list, at `80a5c9e`.** R1: 1,877 unit tests / 219 suites (`:core` 828, `:app` 989, nfc-core 50, nfc-android 10), 0 failures, 0 skips. R2: the whole connected suite on emulator-5554 with the preserved set staged, 218 tests, 0 failures, 0 skips. R3: MCP pytest 324 (56 tools), bundle 158, schedules 109. R4 in R2. R5: three exported activities, other 0. R6: no e-mail, home path, foreign serial or attribution in the range; one author; empty bodies; submodule pin `7e0377a`; `assert(` sweep clean; lint 0 errors (56 warnings, as 1.4.0). CI green on the tip (run 36189740488). Whole-patch review: READY TO TAG, 0 blocking; its one SHOULD is #83 (the MCP `update_schedule(reminders_enabled=True)` re-sends a disabled LOCAL row; next release).

**The direct 1.3.0 → 1.4.1 path, on the emulator.** The production phone was still on 1.3.0, so the path it would take was proven first: the 1.3.0 release asset (sha `a4ca5ec8…3442`) installed fresh; the owner's data loaded with the MCP and loader **of the 1.3.0 tag** (a whole-tree worktree — master's tools refuse schema 7, and the loader imports the MCP in process): the preserved 2026-09-18 export, then the Stage A bundle, then the Stage B manifest, reproducing the production phone's converged state exactly (41 assets, 4 groups, 43 ACTIVE providerless schedules; loader re-plan IDENTICAL 47). The dry-run candidate (same signer as 1.3.0) installed over it in place: identity unchanged, status 1.4.1 / schema 8 / format 8, every 1.3 count identical; master's MCP: plan 43 / 43 / 0 → apply repaired 43 → re-plan 0; 43 rows changed, keys exactly `providers` and `updatedAt`; profiles 36 → 36; loader IDENTICAL 47. The runbook's "R7 in full" gained the standing paragraph for an older-release production phone.

**Release.** Annotated tag `servicetag-v1.4.1` → `80a5c9e`; run 36191044084 completed after the owner's environment approval. Asset `ServiceTag-1.4.1.apk` 11,082,609 B, sha256 `09b7d4fa…b294` equal to the published `.sha256`; exactly one signer, certificate `894afa76…` == `RELEASE_CERT_SHA256` and == the 1.3.0 and 1.4.0 assets; badging 1.4.1 / 17; the eight `uses-permission` lines identical to 1.3.0; not debuggable.

**Development phone.** 1.4.0 / 16 → 1.4.1 / 17 in place (UID, `firstInstallTime` unchanged); R7 (i) every count and per-route hash identical, only `appVersion` differs. MCP plan → 43 / 43 / 0 (48 rows: 43 ACTIVE providerless, 5 ARCHIVED outside the universe); **the owner's tap of `Fix reminder delivery` was the apply** (owner ruling: the shipped repair proves itself and reconciles at once); re-plan 0; 43 rows changed, keys exactly `providers` and `updatedAt`, the 5 archived rows untouched, profiles 36 → 36 identical per row, every rule / policy / postponement / identity field unchanged; the owner confirmed the delivery row gone and the badge cleared; the owner's export (format 8, `scheduleProviders` 43) merge-planned read-only against the same phone: 14 tables, insert 0 / identical 240 / conflict 0.

**Production phone**, after the owner's stop and approval, with real data grown since the 2026-09-24 convergence (51 assets, 31 tags, 36 definitions, 79 profiles, 29 events, 79 attachments, 4 groups, 44 schedules, two of them editor-made with `LOCAL enabled`); the owner's fresh pre-install export archived first. 1.3.0 / 15 → 1.4.1 / 17 in place (identity unchanged); R7 (i) every 1.3 count identical, schema and format 7 → 8, groups identical on the 1.3 key set, the 44 schedule rows keeping ids and contents (one editor-made row's season triple reads through the documented normalisation: FOLLOW_ASSET with no reentry → AT_START, offset 0); R7 (ii) the loader IDENTICAL 44 with 3 ERROR entries that name assets the owner has since renamed (a stale manifest, not the phone's data). MCP plan → 42 / 42 / 0; **the owner's tap**; re-plan 0; 42 rows changed, keys exactly `providers` and `updatedAt`; all 44 `LOCAL enabled`; profiles 37 → 37 identical; every other field unchanged; the owner's export (format 8, `scheduleProviders` 44) merge-planned IDENTICAL: 14 tables, insert 0 / identical 354 / conflict 0. Evidence beside the API snapshots under `noteNFC-backups/` (`dev-r7-*-1.4.1/`, `dev-repair-1.4.1/`, `dev-post-repair-1.4.1/`, `prod-pre-1.4.1/`, `prod-r7-*-1.4.1/`, `prod-repair-1.4.1/`, `prod-post-repair-1.4.1/`), never in this repository.
