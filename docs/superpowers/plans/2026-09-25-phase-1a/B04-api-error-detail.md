...
- **K7.** Season `BadDate(field)` and condition `BadDate`, `BadTime` and `BadTimeZone(field)` set `field = problem.field`. Their messages stay as they are. The maintenance break's `BothOrNeither` (`usecase.SeasonProblem`, raised only by `POST /v1/assets/{id}/maintenance-break`, whose keys are `blackoutStartMmdd` / `blackoutEndMmdd`) answers `blackoutStartMmdd`, its pair's first key by plan §5's rule; the message stays the shipped one. (Controller correction 2026-09-25: an earlier note called it the same value as C6's asset-level `Season.Problem.BothOrNeither`; it is a different type on a different route, found by the implementer.)
...
| the 1.4 rows still null | `SeasonHealthRoutesTest` · `aMalformedValueKeepsTheShippedValidationShape`, extended: `seasonStartMmdd`, `occurredTime` and `tzId` in `field`; the break's `BothOrNeither` → `blackoutStartMmdd`; messages unchanged | leave `field` null |
