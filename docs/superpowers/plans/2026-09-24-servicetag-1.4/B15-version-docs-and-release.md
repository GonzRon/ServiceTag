...
- `docs/design/04-domain-data-model.md` — §15's version plan names schema **8** for seasons, condition and health.
- `docs/release-proofs.md` — R7 gains one generic line: when the release bumps the schema or the backup format, a data archive of the development phone taken before the install merge-plans all IDENTICAL after it (`import_merge(plan_only=True)`), and the status counts of the new tables are reported. Nothing else in the runbook changes; R4 stays `ShareBoundaryTest` alone.
...
| `docs/design/04-domain-data-model.md` §15 | correction | schema **8** carries seasons, condition and health |
| `docs/release-proofs.md` R7 | one line | the pre-upgrade archive merge-plans IDENTICAL after a schema or format bump |
...
None directly. It **asserts** that the numbers behind inv. 62–64 and 124 agree everywhere, and that no design document still states a rule 1.4 retired (spec 1.2 inv. 26; D5 §6).
...
| the data model stale | `VersionAgreementTest` · `theDataModelDocumentNamesTheShippedVersions` (schema 8) | keep 7 |
...
5. **The format-7 import proof** (master §17.8) on `emulator-5554`.
6. R7 on the development phone (master §17.9), with **the pre-upgrade format-7 archive merge-planning IDENTICAL** and the three new count keys at 0.
...
- Anchored: `grep -c 'versionCode = 16' app/build.gradle.kts` → 1; `grep -c 'versionName = "1.4.0"' app/build.gradle.kts` → 1; `grep -cE '^\| 1\.4\.0 \| 16 \|' docs/versioning.md` → 1; `git diff --stat <base> -- app/src/main core/src/main app/schemas tools libs docs/api` → empty (`app/build.gradle.kts` is outside that list and is the one code-adjacent edit).
