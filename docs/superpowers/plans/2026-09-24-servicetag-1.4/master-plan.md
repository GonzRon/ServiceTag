...
**The event offer** (B04, pure; spec §3.3, inv. 93): after a `SEASON_START` or `SEASON_END` event on a MANUAL asset in the opposite phase at `T`, `seasonOfferFor(...)` answers START or END; accepting writes through `RecordSeasonActivation` with the event's date clamped to `[latest row's date, T]` and `eventId` set. Nothing writes unless accepted.
...
`BuildReminderSubjects` stops deriving seasons (`BuildReminderSubjects.kt:138-143, 224-231` go) and maps engine output: ARCHIVED → `Withdrawn`; PAUSED → `Parked(null)`; DORMANT → `Parked(actionableDueOn)` (null for MANUAL); DEFERRED → `Parked(actionableDueOn)`; quiet with a notifying status → `Parked(the first day after the break)`; otherwise `Active`. `RuleFacts.seasonal = servicePolicy ≠ CONTINUOUS` — false before and after on real data, so no content hash moves. **No reminder code evaluates a season, break or policy** (inv. 104; anchored grep).
...
- **Schedule response row.** **Plan decision:** responses carry the backup row **plus** the derived compatibility triple (`seasonBehavior` nullable, `seasonReentry`, `seasonReentryOffsetDays`, from §4's reverse projection) as one flat object, an API-only `ScheduleRowResponse`. That is the single, documented exception to v1's "a response row is the archive row" (spec §8.3 forbids the triple in format 8; §9.1 requires it in responses).
...
- **The error envelope** gains `field: String?` (spec §9.2). **Plan decision:** set to the offending body key for the 1.4 422s that name one key (`POLICY_OFFSET_INVALID` → `policyOffsetDays`, `MANUAL_PHASE_*` → `manualPhase`, `SEASON_WINDOW_*` → `seasonStartMmdd`, `SEASON_DATE_OUT_OF_RANGE` / `CONDITION_DATE_IN_FUTURE` → `occurredOn`, `CONDITION_REASON_TOO_LONG` → `reason`, `FOREIGN_EVENT` → `eventId`, `HEALTH_THRESHOLDS_INVALID` → `criticalFromDays`, `HEALTH_WEIGHT_OUT_OF_RANGE` → `weight`, `HEALTH_PRIMARY_INVALID` → `healthPrimarySubjectId`); null everywhere else.
...
**Plan decision (spec §9.4):** B09 commits `docs/api/command-shapes.json`, one object per command — `asset`, `schedule` (`{keys, legacyKeys, actionFlags, rowToCommand: {assetId: targetAssetId, groupId: targetGroupId}}`), `healthSubject`, `seasonMode`, `maintenanceBreak`, `healthPolicy`, `condition`, `activation` — each listing its request keys. A JVM test asserts each request DTO's serializer descriptor equals its entry; B11's pytest asserts every MCP overlay submits exactly those keys.
...
A brief marked **(UI)** proves only that its surface cannot violate the invariant; unmarked briefs prove it where it is enforced. A brief's own invariant list and this column must agree in both directions.

| inv. | invariant (short) | proved by |
|---|---|---|
| 81 | only an explicit condition write inserts a condition row | B06, B12 (UI), B09 |
...
| 86 | no policy, phase, break or activation writes an event, closure, schedule column or second occurrence | B02, B04, B06 |
| 87 | `rule_changed_at` moves only on a rule change; non-rule edits move nothing and clear no postponement | B01 |
| 88 | the `MM-DD` pair is non-null iff CALENDAR | B01 (migration), B03 (decoder), B04 |
| 89 | activation rows immutable | B01, B09, B14 (UI) |
...
| 95 | the policy moves time-side dates only; meter-only is phase-only; PRE_SERVICE / non-zero offset on it refused; a crossed threshold is DUE | B02, B04 |
...
| 99 | manual START never a boundary; YEAR_ROUND break boundary; PRE_SERVICE refused without one; no strands | B02, B04 |
| 100 | season start never completes, resets or hides pre-service work | B02, B05 |
...
| 103 | DEFERRED after the fold, never notifies, never due, never masks a meter | B02, B07, B13 (UI) |
| 104 | no reminder code evaluates a season, break or policy | B02 |
| 105 | no read path writes `schedule_state` or health | B02, B07, B09 |
| 106 | a group target is CONTINUOUS only | B04, B09, B08 (UI) |
| 107 | conditions immutable; current by the ordering key; no column | B01, B06, B14 (UI) |
| 108 | no UNKNOWN stored | B01, B06 |
| 109 | condition and activation `event_id` are soft links, never owners | B01, B03 |
| 110 | returning to OPERATIONAL inserts one row, earlier rows byte-identical | B06, B14 (UI) |
| 111 | no health value stored or exported | B01, B03, B05 |
...
| 119 | every CRITICAL contributor and DOWN/DEGRADED component on every health surface; no DOWN asset's health without its condition | B07, B09, B12 (UI), B13 (UI), B14 (UI) |
| 120 | a schedule drives ≤ 1 non-archived subject; a subject never changes asset | B06, B03, B10 (UI) |
| 121 | no default threshold, margin or break date; a template fills fields only after confirmation | B06, B04, B08 (UI), B10 (UI) |
...
| 125 | migration and decoder agree row for row, move no `updated_at`; a pre-upgrade format-7 export merges IDENTICAL | B01, B03, B09 (agreement test) |
| 126 | activations and conditions merge only as INSERT, IDENTICAL or CONTENT_DIFFERS; their writes never change the asset row | B03, B04, B06 |
| 127 | no route or tool amends or deletes a condition or activation; condition in no asset command | B09, B11, B01 |
| 128 | legacy inputs translate only via §4.1, never change 1.4-only state (422), a mixed body is 422 | B09, B11, B04 |
| 129 | nothing names an installed component or assembly; nothing reads stock | B15 (release grep) |
| 130 | the schedule-side link guard and its flag | B06, B08 (UI), B09 |
| 131 | snooze never changes health; postpone restarts it from the new actionable date; no journal | B05, B07 |
...
3. **Shape-only rows** (B01's compile-through, B05's rename) are exempt by name: their proof is the shipped suite staying green with only mechanical edits, and the brief lists which shipped tests changed and why.
...
| brief | file | owns |
|---|---|---|
| B01 | `B01-schema-8-and-the-legacy-mapping.md` | domain shapes, the legacy mapping and its golden file, Room 8 + `MIGRATION_7_8` + `8.json`, the three fact/config ports and Room adapters, #64, compile-through, the constructor pre-wiring |
...
| B03 | `B03-backup-format-8-and-merge.md` | format 8, the ≤7 upgrade, merge tables 12–14, export/replace/merge use cases |
...
| B09 | `B09-api-v1.md` | every `/v1` change, `docs/api/v1.md`, `command-shapes.json`, the agreement test |
...
| 2 | **B02** | **B03** | neither (JVM) | `core/schedule/**`, `usecase/RecomputeSchedules.kt`, `core/reminders/BuildReminderSubjects.kt`, `ui/maintenance/DueItemRow.kt`, `ui/components/ServiceTagIcons.kt` vs `core/backup/**`, `core/merge/**`, the four import/export use cases, `VersionAgreementTest.kt`. Neither edits `AppGraph` or the shared fakes |
...
| 7 | **B11** | **B12** | B12 | `tools/servicetag-mcp/**`, `tools/servicetag-schedules/**` vs `ui/theme/SemanticColors.kt`, `ui/components/ServiceTagIcons.kt`, `res/drawable/ic_*` (new), `ui/condition/**`, `ui/health/{HealthWords,HealthBadge}.kt`, `ui/maintenance/{MaintenanceSheet,CompletionFlow}.kt`, `ui/journal/EventEntry*.kt`, `ui/scan/**`, `AppGraph` |
| 8 | **B13** | **B14** | B13, then B14 | `ui/dashboard/**`, `ui/maintenance/{DueItemRow,WhyLines,MaintenanceScreen,MaintenanceViewModel}.kt`, `AppGraph` vs `ui/asset/{AssetDetailScreen,AssetViewModels,AssetMaintenanceSections,AssetsScreen}.kt` (reads graph fields only) |
...
2. **Strings:** every user-visible string added or changed is one of S1–S143, verbatim; a string not in spec §10.7 is a **release blocker**; the three retired strings are absent.
...
8. **Format-7 import proof** on the emulator: a format-7 archive (a fictional fixture holding a CALENDAR asset, a YEAR_ROUND asset, IGNORE and FOLLOW_ASSET schedules including a `RESUME_CLAMPED` and an `MM-DD` re-entry) restores with equal counts, CALENDAR iff both `MM-DD` were set, no break, §4.1's mapping on every schedule, `ruleChangedAt = updatedAt`, and empty new lists; exported again it is format 8 and re-imports IDENTICAL.
9. **R7 — the one signed-APK upgrade smoke,** on the development phone: `adb install -r` of the verified 1.4.0 over the installed 1.3.0, in place, code **15 → 16**, the same UID and `firstInstallTime`; `/v1/status` counts identical before and after on every shipped key, the three new keys 0, `schemaVersion` 8, `backupFormatVersion` 8; spec §12.3's checks (CALENDAR iff both `MM-DD` were set, no break, the §4.1 mapping, empty new lists); **a format-7 data archive of that phone taken before the upgrade merge-plans all IDENTICAL** through `import_merge(plan_only=True)`; attachments and the Stage-C documents survive (counts).
...
- the numbers — `grep -c 'version = 8' app/src/main/kotlin/com/loosecannon/servicetag/data/room/AppDatabase.kt`, `grep -c 'SCHEMA_VERSION = 8' app/src/main/kotlin/com/loosecannon/servicetag/di/AppGraph.kt`, `grep -c 'FORMAT_VERSION = 8' core/src/main/kotlin/com/loosecannon/servicetag/core/backup/BackupCodec.kt`, `grep -c 'versionCode = 16' app/build.gradle.kts`, `grep -c 'versionName = "1.4.0"' app/build.gradle.kts` → **1** each
...
16. Counted days range over `O < d ≤ T` (§10.2).
...
22. The schedule response is the archive row plus the derived triple, `ScheduleRowResponse` (§11.3).
23. The `field` envelope names the offending key for the listed codes (§11.3).
...
29. `<age>` in S99 renders as the number of days; numbers substitute without pluralisation (B12).
...
33. A health subject name over 60 characters is `HEALTH_SUBJECT_NAME_REQUIRED`, the spec's one name code (B06).
