...
16. Counted days range over `min(O, P ?: R) < d ≤ T` (§10.2; **changed by the controller's ruling on I7** — `O < d` under-counted a never-terminated COMPLETION schedule anchored before its rule-change date).
