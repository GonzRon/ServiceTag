...
- `core/.../core/usecase/SeasonEventOffer.kt` — `seasonOfferFor(asset, activations, event, today): SeasonAction?` and `acceptSeasonOffer(...)`.
...
- `app/src/main/kotlin/com/loosecannon/servicetag/di/AppGraph.kt` — `setSeasonMode`, `setMaintenanceBreak`, `recordSeasonActivation`, `getAssetSeason`; the re-wired `updateAsset` and `createAsset`.
- `app/src/test/.../testing/FakeGraph.kt`, `core/src/test/.../testing/InMemoryRepositories.kt` — only if a fake needs a query this brief adds.
...
3. **The strands rule** — master §7.2: refuse (409) a season-mode or break change that changes `boundaryKindOf(mode, hasBreak)` while **any** PRE_SERVICE schedule targets the asset, whatever its lifecycle. Changing a CALENDAR window, or a break's dates, is allowed. The legacy pair obeys the same rule.
...
7. **The season event offer** — `seasonOfferFor` answers START after a `SEASON_START` event, END after a `SEASON_END` event, only on a MANUAL asset whose phase at `today` is the opposite; else null. `acceptSeasonOffer` calls `RecordSeasonActivation` with the event's date clamped to `[the latest row's date, today]` and `eventId` set. **No journal event ever changes a phase by itself** (inv. 93).

## Invariants this brief must hold

**86** (season and break commands write no event, closure or schedule column), **88**, **91**, **92**, **93** (predicate half), **95** (refusal half), **99** (refusal half), **106**, **121** (no break default), **126** (activations never touch the asset row), **128** (the asset pair on MANUAL).
