...
4. **Counted days** — `d` over `min(O, P ?: R) < d ≤ T` (master §20.16, **changed by the controller's ruling on I7**; the predicate `d > A(d)` bounds it from below) counts when `evaluate(link.inputs, season, at = d)` has phase ACTIVE, a non-null `actionableOn`, and `d > actionableOn`; and, for a MEDIUM subject on IN_SERVICE_*, `d ≥ season.latestCycleStartOnOrBefore(T)` when that exists. The production code computes spans between phase, cycle and break boundaries; a **day-by-day oracle lives only in the test** and the two must agree. **Owner ruling (2026-09-24):** the range is an evaluation bound only. After a postponement, MAINTENANCE_OVERDUE counted lateness is zero until the policy-derived postponed actionable date has passed; no day before that actionable date contributes to health. `A(d)` is therefore always evaluated with today's postponement, never as it stood on `d`.
...
**Accountable** (master §14): **82** (the engine writes nothing — it has no port), **111**, **112**, **113**, **114**, **115**, **116**, **117**, **118** (untracked subjects excluded from aggregation and never 100), **131**. **Halves:** **100** (the clock).
...
| the clock follows a raw date | `HealthClockTest` · `theSpanImplementationEqualsTheDayByDayOracle` (property over generated policies, contexts, postponements and `O`, **including histories with `R < O`**) | measure from `computedDueOn` |
| a postponement leaves old lateness behind | `HealthClockTest` · `noDayBeforeThePostponedActionableDateCounts` (a schedule 40 days late is postponed today to T + 10: counted days are 0 today and 1 on T + 11) | evaluate `A(d)` with the postponement as it stood on `d` |
| lateness before `O` lost (I7) | `HealthClockTest` · `aCompletionScheduleAnchoredBeforeItsCreationCountsFromTheAnchor` (never terminated, anchored 30 days before creation: `x = T − anchor`, not `T − O`) and `aMigrated13RowWhoseUpdatedAtMovedAfterItsAnchorStillCountsFromItsDue` (a row whose `rule_changed_at`, seeded from 1.3's `updated_at`, is later than its raw due) | start the range at `O` |
