...
## 4. The strings — RATIFIED by the owner on 2026-09-25 (rulings.md, adjudication)
...
7. **Development phone** (1.4.0 / 16 → 1.4.1 / 17): R7 in full — `adb install -r` in place, `firstInstallTime` and UID unchanged, R7 (i) counts and per-table hashes identical on the 1.4 key set. **The owner does not open Reminder Health's new button before step 8 is recorded** (its tap is the apply). 8. MCP plan → 43 (the population the forensic inventory named). 9. **the owner taps `Fix reminder delivery`** (owner ruling, step-12 option iii: the shipped repair is the apply and reconciles at once; the MCP apply stays the fallback). 10. re-plan → 0. 11. `list_schedules` before/after: every row's id, `profileId`, rule, policy, `postponedDueOn`, `ruleChangedAt` identical; only `providers` and `updatedAt` moved on the 43; closures and events counts identical; the 5 ARCHIVED rows still providerless. 12. The owner's tap reconciled at once, so LOCAL delivery is live from step 9; the Reminder Health screen shows neither delivery sentence (P141-1 nor the residual) and the `REMINDER FAILED` badge clears unless another WARN finding stands (the owner's look at the stop); `BuildReminderSubjectsTest` stands for the subjects. The `/v1` apply path is proven on the emulator (step 4). 13. **Post-repair backup (R5), proven:** the owner makes one export from Settings on the development phone into its usual folder; the controller runs `import_merge(archive, plan_only=True)` against the same phone — every table `insert 0 / conflict 0`, every exported row `IDENTICAL` (attachment rows `skipped` only where bytes are absent) — reads `scheduleProviders` = **43**, and archives a copy under `noteNFC-backups/dev-post-repair-1.4.1/` beside the before/after API snapshots. 14. **STOP** for owner approval.
15. **Production phone**, only after the stop, the direct path already proven: the owner grants INTERNET for the session (revoked after, as the owner prefers); install 1.4.1 over 1.3.0 (code 15 → 17) in place; R7 (i) on the 1.3 key set with the 1.4 mapping **and** R7 (ii), the loader re-plan IDENTICAL (4 groups + 43 schedules), because this install crosses schema 7 → 8; the same no-tap-before-the-plan instruction as step 7. 16. plan → the count it reads (43 by provenance; the plan is the read-only confirmation #80 asks for). 17. **the owner taps `Fix reminder delivery`** (option iii). 18. re-plan 0 and the step-11 invariants. 19. the owner's export into the production folder, proven exactly as step 13, archived under `noteNFC-backups/prod-post-repair-1.4.1/`, and the evidence.
...
- **Strings §4:** ratified 2026-09-25. Wave 1 approved.
- **Q1 — RESOLVED:** option (iii), see §8 steps 9 and 17.
- **Q2 — RESOLVED:** PATCH `"providers": null` stays a 400 (P1 pins it).
- **Q3 — RESOLVED:** the rewritten README and the re-anchored tests stay.
