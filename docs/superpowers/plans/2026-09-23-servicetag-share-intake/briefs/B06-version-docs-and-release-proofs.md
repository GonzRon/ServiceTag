...
- `grep -c 'android.intent.action.SEND_MULTIPLE' app/src/main/AndroidManifest.xml` → **0**; `grep -c 'android.intent.category.BROWSABLE' app/src/main/AndroidManifest.xml` → **1** (§18.24: the shipped `servicetag://` deep-link filter on `MainActivity`, present since 1.2), and `git diff <base>..HEAD -- app/src/main/AndroidManifest.xml | grep -c '^+.*BROWSABLE'` → **0** — 1.3 adds none, and B03's filter `@Test` asserts the share activity's filter carries no `BROWSABLE`
