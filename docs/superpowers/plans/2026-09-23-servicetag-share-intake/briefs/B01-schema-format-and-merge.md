...
`api/**` file but the two edits above; `tools/`.
...
- `./gradlew :app:testDebugUnitTest --console=plain` — green, with `ReferenceMigrationTest` and `ReferenceDaoConstraintTest` present and counted. **`VersionAgreementTest` will be red on the schema/format cases** until B06 lands the version row; the implementer reports that as expected and does **not** touch `app/build.gradle.kts` or `docs/versioning.md` to make it green.
...
- Report the per-module test deltas (`:core` before/after, `:app` before/after) in the report file.
