...
### 18.24 Two B03 corrections found by implementation (2026-09-23)

- **The file-wide `BROWSABLE` count of 0 was never satisfiable.** The shipped `servicetag://` deep-link filter on `MainActivity` has carried `android.intent.category.BROWSABLE` since 1.2, so `grep -c` over the source manifest is **1** at every base. The gate in B03 and in B06's release proofs is corrected to the **diff-scoped** count (`^+.*BROWSABLE` → 0 against the lane's `$BASE` in B03, against `<base>` in B06) plus the file-wide **1**; the §16 row "never declared" was always about the share activity's own filter, which B03's parsing `@Test` asserts. No spec change: §5's exported-component rule speaks of the share activity, not the manifest.
- **Two shipped assertions outside B03's Files list move, strengthened.** `ScheduleDeepLinkTest`'s pinned `<intent-filter>` count 7 → 8 and `RemovedSurfacesTest`'s shared-text assertion re-pinned to exactly one `ACTION_SEND` handler and zero `ACTION_SEND_MULTIPLE`; both are now named in B03's Modify list, as B01's §18.19 did for its format-number pins. The Files-list rule (§1.6) is served by naming them, not by leaving a shipped test red.
- **Two controller rulings on spec silence, recorded for the owner (non-blocking):** (a) `OwnerMissing` — the chosen asset deleted while the intake screen is open — draws the ratified no-assets dead end ("Add an asset in ServiceTag first, then share this again." / Close) rather than an unratified sentence; the owner may ratify a dedicated line later. (b) A `text/uri-list` stream with **no** usable line (only blanks and `#` comments) is `Refused(UNREADABLE)` → "Could not read what was shared": the list carried nothing, and the not-a-link path's "Save as a note" must never offer to store a uri-list's bytes (B03 §"uri-list never reaches the attachment path").

### 18.25 R4 mechanism corrected while running it (controller, 2026-09-23)

- **The documents-provider URI does not carry a shell grant on the emulator image in use (Android 17, API 37.1).** `am start … --eu android.intent.extra.STREAM content://com.android.externalstorage.documents/… --grant-read-uri-permission` is refused by `UriGrantsManagerService.enforceRequireContentUriPermissionFromCallerOnIntentExtraStream`, so the intake correctly answers "Could not read what was shared". R4's byte-share flows therefore use a **MediaStore URI** — `content://media/external/downloads/<id>` for the pushed PDF (the push is scanned automatically; the id comes from `content query --uri content://media/external/downloads --projection _id:_display_name`) and `content://media/external/images/media/<id>` for the image — which carries the grant and renders the byte-share form. B06's runbook paragraph for flow 2 is superseded by this note; the flows and their expectations are unchanged.
- **adb key injection reaches no Compose text field on that image** (`input text`, key events; window focused, IME shown; the Assets search box fails the same way), so the intent-driven flows that need a typed name (1, 6, 10b) share **Markdown-labelled links** (`[Manual](https://…)`), whose prefilled name is itself flow 4's proof; the typed-name path stays proven by the connected `ShareIntakeScreenTest`. Not an app defect.
- Added to R4: a **process-death** flow (force-stop after choosing the asset) leaving nothing written — the whole-branch reviewer's named proof gap.

### 18.26 §18.25 corrected after the owner's research (2026-09-23, later the same day)

- **The documents-provider diagnosis in §18.25 was imprecise.** The shell (uid 2000) cannot *delegate* a Storage Access Framework URI it does not itself hold a grant for; `UriGrantsManagerService` checks that the sender is entitled to the URI before letting it pass a temporary grant on, so `adb shell am start … --grant-read-uri-permission` with `content://com.android.externalstorage.documents/…` does not reproduce what the Files app does (Files holds the SAF grant; the shell does not). That is a limitation of the test **sender**, not of ServiceTag or of ordinary Android sharing. The `enforceRequireContentUriPermissionFromCallerOnIntentExtraStream` line in logcat is observational when the activity leaves `requireContentUriPermissionFromCaller` at its default `none`; it is not the denial. R4 keeps the **MediaStore URI** as the deterministic shell-driven path (the shell can read it, so the real `content://` + temporary grant + copy path runs) and adds **one real Files-app share** of the fixture, driven through DocumentsUI's own share sheet, as the proof that a genuine SAF grant reaches the intake form (flow R4.F).
- **The typing diagnosis in §18.25 was wrong.** adb key injection was reaching **Gboard's stylus-handwriting onboarding** ("Try out your stylus"), which the emulator triggers because it advertises a stylus path; the injected characters landed in Gboard's own try-out box and the dispatcher logged `VRI[InputMethod]: Cancelling event (no window focus)`. With `settings put secure stylus_handwriting_enabled 0` on the emulator, `input keyboard text` and `input text` both reach the intake's Compose field (verified: "abcxyz123"). Not an emulator input-stack defect and not an app defect. R4's flows 1, 6 and 10b are back to **bare URLs with the name typed**, as the runbook says; the harness disables stylus handwriting in its setup step; the labelled-link variant is withdrawn.
- Lesson recorded in the ledger: a diagnosis is not a conclusion until an A/B against a non-app surface has been run.
