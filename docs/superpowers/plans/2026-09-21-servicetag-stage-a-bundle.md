...
- Python golden: `archive_bytes(load_source(fixture))` equals the committed resource byte-for-byte, and the test **fails** (never skips) when the resource is missing — so a generator change that alters output fails CI until the fixture is deliberately regenerated and both files committed together. The README documents that regeneration step. The fixture's `namespace` is a fictional literal.
