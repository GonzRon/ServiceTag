...
`ScheduleRecompute.rebuild(schedule, events, closures, membership, today, zone, season = null)` is the **only** write path into `schedule_state` (invariant 17). It runs after every event insert/update/delete, every closure insert, every schedule edit, every import, and in the digest and backstop runs. It is **pure** in (config, events, closures, membership, `T`, the zone `T` is on) and **idempotent** (invariants 15, 16 — asserted as properties over the function, not with two devices).

> **Signature amendments.** Planned as `rebuild(schedule, events, closures, membership, T)`. At B02's review (2026-09-22) it gained a trailing, defaulted `season: SeasonWindow? = null` (the matrix's `Season.inSeason` row needs it; a missing window fails open; callers that know the window pass it). At the release gate (2026-09-22) `zone` became a required pure input between `today` and `season` — the device zone, supplied by `RecomputeSchedules`, read only by the D-27 floor — so `season` is the seventh parameter.
...
|---|---|---|
| permission | `POST_NOTIFICATIONS` only, requested **on first schedule creation with a rationale** — not at launch. The request lives in the editor (B14) with B05's plumbing | §5.1, #24 AC 1 **Amended (owner ruling, 2026-09-22):** "only" means the only new *runtime-requested* permission; `RECEIVE_BOOT_COMPLETED` (a normal, install-time permission, no prompt) is declared by B05 because invariant 60's re-arm after `BOOT_COMPLETED` is undeliverable without it. Caveat, documented not gated: on Android 13+ an app in the restricted background state may not receive `BOOT_COMPLETED` until launched again — `APP_RESTRICTED` is a health finding (B10), not something the app repairs. |
...
4. **Structural greps.** Every one is an **anchored pattern with its command and its expected count**, per `docs/superpowers/planning-policy.md:41` — written this way from the start so a comment that names a grep can never match it. `<base>` is the release branch's base commit. **They are a list and not a table on purpose: an ERE alternation contains a `|`, which a markdown table cell would require escaping as `\|`, and a pattern that has to be un-escaped before it runs is exactly the silently-matching-nothing failure the anchored-pattern rule exists to prevent.** Run each line as written.
   ...
   - **no exported new receiver** — `grep -cE '<receiver' app/src/main/AndroidManifest.xml` → **6** (the manifest declares none today; §12 adds the four platform receivers, B06's `DigestReceiver` — **amended 2026-09-22:** the alarm's own receiver must be manifest-declared, which spec §5.2 implies and §5.4's enumeration omitted — and B07's quick-action receiver), and `grep -cE '<receiver[^>]*android:exported="true"' app/src/main/AndroidManifest.xml` → **0**, with the per-element assertion in B05's test
...
6. **End-to-end on `emulator-5554`** through a connected MCP client with the Developer API screen open: create a group with two members, create a group-targeted QUICK schedule, `list_due` shows **one** item with `membersRequired = 2, membersComplete = 0`; complete one member → `membersComplete = 1` and the item still due; `close_round` → 201, a second `close_round` → 409 `OCCURRENCE_ALREADY_CLOSED`, `complete_schedule` on that occurrence → 409 `OCCURRENCE_CLOSED`; `list_closures` shows one row; `closedOn` tomorrow → 422 `CLOSED_ON_OUT_OF_RANGE`; export, `import_merge(plan_only=True)` on the same phone → all `IDENTICAL` including the schedule and the closure; `pm clear` after. **Corrected at the release gate (2026-09-23, controller ruling):** by spec §2.4 the next round opens the moment the previous one is closed (its open instant is the closure's `createdAt`), so in this sequence a second `close_round` closes the *next* round — a closure for a later `occurrenceOn`, never a duplicate — and a completion sent after the close is an early completion of the round now current; `OCCURRENCE_ALREADY_CLOSED` and `OCCURRENCE_CLOSED` need a closure on the *current* key and are proved by `CloseRoundTest.closingTwiceIsRefusedAndTheFirstRowStands`, `MaintenanceRoutesTest.closingTheSameRoundTwiceIs409OccurrenceAlreadyClosed` and `…completingAClosedRoundIs409OccurrenceClosed`. The proof asserts the spec's behaviour; the two 409 expectations above are withdrawn.
...
| # | brief | decision |
|---|---|---|
| 17 | B01 | **The Room adapter for a port belongs to the brief that declares the port** — `ScheduleRepository`/`ScheduleStateRepository` to B02, `GroupRepository`/`ClosureRepository` to B03, `ScheduleLocalDeliveryRepository` to B06. Spec §7's `(:core)` label says where the *domain* lives, not that an adapter is unowned; B01 declares the DAO operations those adapters call **Amended at B01's review (2026-09-22, controller ruling R1):** B01 had to declare `GroupRepository`, `ScheduleRepository` and `ClosureRepository` (its merge use cases take them) and wrote their minimal Room adapters `RoomGroupRepository` / `RoomScheduleRepository` / `RoomClosureRepository` in `app/.../data/room/MaintenanceRepositories.kt` plus three `AppGraph` fields, because `:app` cannot compile otherwise. B02 and B03 therefore **extend** those ports and adapters with their operations (B02 adds `ScheduleStateRepository` and its adapter; B03 may keep its two adapters in `MaintenanceRepositories.kt` or move them to `GroupRepositories.kt`); they do not create them. |
