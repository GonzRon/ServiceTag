...
| `NO_DATA` | a schedule with a meter rule and **no baseline** — neither a completion nor `anchorMeter` | WARN | `OpenInApp` — the completion / anchor form, offered as the ratified **"Log meter reading"** |
...
| `NO_DATA` | **positive:** a meter rule with no completion and no `anchorMeter` → the finding with a repair that opens the anchor form (#27 AC 5). **Control:** the same schedule with `anchorMeter` set → silent | a schedule that can never become due sits in ATTENTION with nothing offered to fix it |
...
**PROPOSED — this brief's defining string obligation** (spec §12 item 1, master plan §17): **one plain sentence and one repair label for each of the seven findings** — fourteen strings. Draft them in the register of the shipped screens: say what is wrong in the owner's terms, not the platform's; name the consequence ("reminders will not arrive") rather than the mechanism; and make the repair label a verb. Hand all fourteen to the controller **with this brief's plan review**, and **do not execute this brief until the owner has ratified them**. This is the one brief whose execution the string gate genuinely blocks.
