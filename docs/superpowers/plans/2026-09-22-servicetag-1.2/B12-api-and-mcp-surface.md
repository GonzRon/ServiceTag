...
| an overlay writing past validation | an overlay built from a row carrying a stale or optional field still round-trips through the **owning use case's** validation, not a raw upsert — so a read-back-and-resubmit is re-validated as a fresh command | a handler that upserts the overlaid row directly would let a legacy or hand-edited row bypass the rules `SaveSchedule`/`SaveGroup` enforce; the route-to-use-case table above is what makes this structurally unreachable, and this row proves it |
