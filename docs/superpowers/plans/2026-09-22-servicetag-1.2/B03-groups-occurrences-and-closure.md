...
| "Complete all" twice | "Complete all" writes one event per not-yet-completed required member in **one** `uow.write`; running it again writes **nothing** and the event count is unchanged (invariants 31, 32) | without the unique index the second run duplicates every member's event |
...
| closing twice | the second close is **refused** and the **first row stands**, unchanged (invariant 38, T10) | an upsert overwrites the first closure's date and the recurrence silently moves |
...
- Structural, anchored:
  ...
  - `grep -rn 'removed_at\|removedAt' core/src/main | grep -iE '= *null|clear'` → no site that clears it (invariant 79).
