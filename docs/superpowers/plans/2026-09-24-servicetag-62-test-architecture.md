...
- **R5** names `ManifestContractTest`, the file's first class, with the merged-manifest assertions credited to `MergedManifestContractTest` — which **does exist** as the second class in `ManifestContractTest.kt` (line 318). An earlier version of this line called that name stale; it was the controller's error, the same one the 1.3 master plan §18.5 had already recorded once.
