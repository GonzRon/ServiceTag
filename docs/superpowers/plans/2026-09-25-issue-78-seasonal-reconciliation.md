...
`ServiceTagRoot` for the shape); tests `AssetViewModelsTest` (JVM) and `AssetEditorSeasonAndHealthTest`
...
- Tests: `app/src/test/…/ui/asset/AssetViewModelsTest.kt` (the construction helper at `:112` gains `graph.schedules`; the new cases), `app/src/androidTest/…/ui/asset/AssetEditorSeasonAndHealthTest.kt` (the dialog), `app/src/androidTest/…/ui/asset/AssetDetailConditionHealthSeasonTest.kt` (the section scroll, screen-only), a new `app/src/androidTest/…/ui/asset/SeasonReconciliationNavigationTest.kt` (the real back stack: it composes `ServiceTagRoot` exactly as `ui/scan/ReaderModeHoldTest.kt` does — `rule.setContent { ServiceTagRoot(graph, deepLinks, snackbars) }` with a real `MutableSharedFlow<Route>` — because a screen composed alone has no stack and the nav entry decorator owns the view-model store, as `InspectNamesABoundTagTest.kt:58-62` records), `core/src/test/…/core/schedule/ScheduleStatusTest.kt` (AC 2; if `NoBacklogAcrossDormancyTest` already pins the exact case, cite it instead of adding).
...
| the dialog | `AssetEditorSeasonAndHealthTest` · `switchingAYearRoundAssetToManualAsksAboutItsContinuousSchedules`: an asset with one CONTINUOUS schedule; choose Started and ended by hand, answer S35, Save; P78-1b and both buttons displayed; tap Keep → the editor closes | drop the dialog |
| the back gesture | `AssetEditorSeasonAndHealthTest` · `theBackGestureOnTheSeasonPromptKeepsTheSchedules`: press back on the dialog → the editor closes through `saved`, `review` never fires, nothing written | wire `onDismissRequest` to `reviewSchedules()` |
| the section scroll | `AssetDetailConditionHealthSeasonTest` · `theSchedulesSectionScrollsIntoViewWhenAsked`: `AssetDetailScreen(assetId, section = "schedules")` composed alone (the class's own pattern) shows `AssetMaintenanceSections` without a tap | ignore `section` |
| Review lands on the schedules, one detail on the stack (C4) | `SeasonReconciliationNavigationTest` · `reviewMaintenanceSchedulesLandsOnTheAssetsSchedulesOnce`: `ServiceTagRoot` composed; an existing YEAR_ROUND asset with one CONTINUOUS schedule; open it, Edit, choose Started and ended by hand, answer S35, Save, tap Review → the maintenance sections are displayed; press back **once** → the app has left the asset entirely (the list it started from), never a second, stale detail | add instead of replace in `onReviewSchedules` |
...
- Connected, on `emulator-5554` only, one class per invocation: `com.loosecannon.servicetag.ui.asset.AssetEditorSeasonAndHealthTest`, then `com.loosecannon.servicetag.ui.asset.AssetDetailConditionHealthSeasonTest`, then `com.loosecannon.servicetag.ui.asset.SeasonReconciliationNavigationTest`.
...
- `git diff <base> -- core/src/main tools docs/api app/src/main/kotlin/com/loosecannon/servicetag/api app/src/main/kotlin/com/loosecannon/servicetag/ui/maintenance app/src/main/kotlin/com/loosecannon/servicetag/di app/src/main/AndroidManifest.xml app/src/test/kotlin/com/loosecannon/servicetag/testing` → empty.
