...
centrally, for all 56 tools at once — see their docstrings for the two-layer why. This file proves
...
# --- condition 1: every one of the 56 tools publishes additionalProperties: false -----------------
...
def test_a_parameter_that_shadows_a_basemodel_attribute_is_listed_by_its_real_name() -> None:
    ...
    must stay 56 (asserted at the end, the same way condition 5 pins it elsewhere)."""
    ...
    assert len(server_module.mcp._tool_manager.list_tools()) == 56


# --- fix spec 4(c): parametrised over every one of the 56 registered tools ------------------------
...
def test_the_tool_count_is_56() -> None:
    assert len(server_module.TOOL_NAMES) == 56
    assert len(server_module.mcp._tool_manager.list_tools()) == 56
...
def test_the_guard_raises_when_a_genuine_arg_models_rebuild_does_not_change_validation() -> None:
    ...
    **no rebuild at all** already makes its schema say `additionalProperties: false` — while
    ...
    other test and none of the real server's 56 already-guarded tools are affected. The guard must
