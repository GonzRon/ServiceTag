...
class ScheduleOperationsTest {
    ...
    fun completingWritesTheScheduleRowOnlyToClearAPostponement() = runTest {
        ...
        assertTrue(cleared.updatedAt > postponed.updatedAt, "clearing the postponement bumps the stamp")
    }
    ...
    /**
     * **Edit recurrence**: the rule columns, the postponement cleared, the floor moved to the edit
     * date — and **history untouched**. The recorded completion and the closure stay exactly as
     * they were: an edit abandons an open occurrence by leaving its rows alone and letting the new
     * rule produce the next one, and "tidying up" old rows whose keys no longer lie on the series
     * is the bug (invariant 14). A title-only edit clears nothing.
     *
     * The partially-complete case belongs to a group occurrence, and that is the groups brief's.
     */
    @Test
    fun aRecurrenceEditClearsThePostponementMovesTheFloorAndRewritesNoHistory() = runTest {
        val assetId = seedAsset()
        val schedule = save.run(null, quarterlyCommand(assetId))
        closures.insert(
        ...
        // the closure's key is still on the old series and is left exactly where it is; the new
        // due date is computed against the new series from the closure's own effective date
    ...
    /**
     * Spec §3.2: derived state is rebuilt after **any** import, and a replace import is the one
     * that replaces the most — the wipe takes `schedule_state` with it through the CASCADE. Without
     * the rebuild the table is simply empty afterwards and every schedule has no due date at all,
     * which is what this asserts by rebuilding the same file into the same store.
     */
    @Test
    fun aReplaceImportRebuildsDerivedState() = runTest {
        val assetId = seedAsset()
        val schedule = save.run(null, quarterlyCommand(assetId))
        assertEquals("2026-04-01", states.get(schedule.id)!!.computedDueOn)
        ...
        ImportBackupReplace(
            assets, groups, tags, links, defs, profiles, schedules, closures, events, attachments,
            storage, uow, rebuildAll = { recompute.all() },
        ).run(bytes)
